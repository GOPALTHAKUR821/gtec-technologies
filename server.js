...
async function rpc(action, payload = {}) {
  const { data, error } = await supabase.rpc('gtec_api', { p_action: action, p_payload: payload });
...
app.post('/api/login', async (req, res) => {
  try {
    const data = await rpc('login', { username: clean(req.body?.username, 100), password: String(req.body?.password || '') });
    res.json(data);
  } catch (e) { errorResponse(res, e); }
});

app.post('/api/logout', auth, async (req, res) => {
  try { await rpc('logout', { token: req.adminToken }); res.json({ ok: true }); }
  catch (e) { errorResponse(res, e); }
});
...
app.get('/api/products', async (req, res) => {
  try { res.json(await rpc('products')); }
  catch (e) { errorResponse(res, e); }
});

app.get('/api/admin/products', auth, async (req, res) => {
  try { res.json(await rpc('admin_products', { token: req.adminToken })); }
  catch (e) { errorResponse(res, e); }
});

app.post('/api/admin/products', auth, async (req, res) => {
  try { res.json(await rpc('create_product', { token: req.adminToken, ...(req.body || {}) })); }
  catch (e) { errorResponse(res, e); }
});

app.put('/api/admin/products/:id', auth, async (req, res) => {
  try { res.json(await rpc('update_product', { token: req.adminToken, id: req.params.id, ...(req.body || {}) })); }
  catch (e) { errorResponse(res, e); }
});

app.delete('/api/admin/products/:id', auth, async (req, res) => {
  try { res.json(await rpc('delete_product', { token: req.adminToken, id: req.params.id })); }
  catch (e) { errorResponse(res, e); }
});

app.post('/api/orders', async (req, res) => {
  try { res.json(await rpc('create_order', req.body || {})); }
  catch (e) { errorResponse(res, e); }
});

app.post('/api/track-order', async (req, res) => {
  try {
    res.json(await rpc('track_order', {
      ...(req.body || {}),
...
app.get('/api/admin/orders', auth, async (req, res) => {
  try { res.json(await rpc('admin_orders', { token: req.adminToken })); }
  catch (e) { errorResponse(res, e); }
});

app.get('/api/admin/tracking-stats', auth, async (req, res) => {
  try { res.json(await rpc('tracking_stats', { token: req.adminToken })); }
  catch (e) { errorResponse(res, e); }
});
...
app.delete('/api/admin/orders', auth, async (req, res) => {
  try { res.json(await rpc('delete_orders', { token: req.adminToken, ids: Array.isArray(req.body?.ids) ? req.body.ids : [] })); }
  catch (e) { errorResponse(res, e); }
});

// Frontend fallback. API routes above are handled before this.
