...
// Disable cache for API and admin page
...
// Existing orders ke liye status_updated_at set karna
db.prepare(`
  UPDATE orders
  SET status_updated_at = COALESCE(status_updated_at, date)
...
function productPayload(body, existing = {}) {
  ...
  return {
    ...
    category: cleanText(
      body.category ?? existing.category ?? 'Camera',
      80
    ),
...
function delivery(pin, city) {
  ...
  if (
    ['202', '281', '282', '283']
      .some(prefix => pin.startsWith(prefix))
  ) {
    return 70;
  }
...
app.post('/api/orders', (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({
        error: 'Delivery details required'
      });
...
// API routes ke alawa agar koi frontend route hai,
